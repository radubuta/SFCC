...
server.get('Show', cache.applyPromotionSensitiveCache, function (req, res, next) {
    ...
    var excludefromplpList = Site.getCurrent().preferences.custom.excludefromplp
    ...
    var listProducts = excludefromplpList.split(',');
    ...
    var isVisible = (listProducts.indexOf(product.id) > -1);
    if (isVisible) {
        context = {
            product: {},
            urls: {
